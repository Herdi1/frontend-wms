  ...
  <div class="col-md-1 mt-2">
    <button class="btn btn-sm btn-primary"
        ...
        <div class="mt-2"
          ...
          <div class="btn-group border " v-if="isMaintenancePage">
            <button class="btn btn-sm"
              :class="self.parameters.params.soft_deleted == 'all' ? 'btn-primary active' : ''"
              @click="self.parameters.params.soft_deleted = 'all';self.onLoad()"
              data-toggle="tooltip"
              data-placement="top"
              data-original-title="Semua Data">
              <i :class="['fas fa-font',{'text-primary' : self.parameters.params.soft_deleted != 'all'}]"></i>
              <!-- <span class="badge badge-danger">{{total}}</span> -->
            </button>
            <button class="btn btn-sm"
              :class="self.parameters.params.soft_deleted == '' ? 'btn-info active' : ''"
              @click="self.parameters.params.soft_deleted = '';self.onLoad()"
              data-toggle="tooltip"
              data-placement="top"
              data-original-title="Data Active">
              <i :class="['fas fa-list',{'text-info' : self.parameters.params.soft_deleted != ''}]"></i>
             <!-- <span class="badge badge-danger">999</span> -->
            </button>
            <button class="btn btn-sm"
              :class="self.parameters.params.soft_deleted == 'deleted' ? 'btn-danger active' : ''"
              @click="self.parameters.params.soft_deleted = 'deleted';self.onLoad()"
              data-toggle="tooltip"
              data-placement="top"
              data-original-title="Data Terhapus">
              <i :class="['fas fa-trash',{'text-danger' :self.parameters.params.soft_deleted !== 'deleted'}] "></i>
          ...
          <div class="btn-group float-right" v-if="isExport">
            <button class="btn btn-sm btn-primary btn-block"
              ...
              @click="exportFile({type : 'excel',parameters : self.parameters})"
