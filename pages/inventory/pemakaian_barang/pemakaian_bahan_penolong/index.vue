...
export default {
  ...
  head() {
    return {
      title: "Pemakaian Bahan Penolong",
    };
  },
    ...
    getRoles() {
      if (!this.user.parent_id) {
        ...
        let main_role = this.user.group_role.roles.find(
          (item) => item.name == "pemakaian_barang"
        );
  ...
  methods: {
    ...
    onFormShow() {
      this.$router.push("/inventory/pemakaian_barang/pemakaian_bahan_penolong/add");
    },

    onEdit(id) {
      this.$router.push("/inventory/pemakaian_barang/pemakaian_bahan_penolong/" + id);
    },
