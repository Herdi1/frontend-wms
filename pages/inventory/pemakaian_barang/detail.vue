            ...
            <div class="receipt-main">
              ...
              <div class="row mt-2">                               
                <div class="col-md-12 float-right ">
                  <div class="receipt-right text-right">
                    ...
                    <p><b>Tanggal :</b> {{ parameters.form.tanggal }} </p>
                    <!-- <p><b>Kode Permintaan Material :</b> {{ parameters.form.material_requirement ? parameters.form.material_requirement.code : "-" }}</p> -->
              ...
              <div>
                <table class="table table-striped table-sm vld-parent">
                  ...
                  <tbody>
                    <tr v-for="item in parameters.form.detail_pemakaian_barangs" :key="item.id">
                      <td>{{ item.product ? item.product.code : '-' }}</td>
                      <td>{{ item.product ? item.product.name : '-' }}</td>
                      <td>{{ item.grade ? item.grade.code : '-' }}</td>

                      <td>{{ item.quantity | formatPrice }}</td>
                    </tr>

                    <tr v-if="!parameters.form.detail_pemakaian_barangs.length">
