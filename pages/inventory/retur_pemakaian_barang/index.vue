...
export default {
    ...
    getRoles() {
      if (!this.user.parent_id) {
        ...
        let main_role = this.user.group_role.roles.find(
          (item) => item.name == "retur_pemakaian_barang"
        );
  ...
  methods: {
    ...
    onFormShow() {
      this.$router.push("/inventory/retur_pemakaian_barang/add");
    },

    onEdit(id) {
      this.$router.push("/inventory/retur_pemakaian_barang/" + id);
    },
    ...
    onPrintDetail(item) {
      ...
      window.open(
        process.env.API_URL +
          "/inventory/pemakaian_barang/get-print-detail/" +
          item.id +
          "?token=" +
          token,
        "_blank"
