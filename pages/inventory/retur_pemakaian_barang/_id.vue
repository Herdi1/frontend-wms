<template>
        ...
        <div class="card-body">
          <div class="card-title">
            <div class="d-flex justify-content-between">
              <h6>
                {{ isEditable ? "Edit" : "Tambah" }} Retur Pemakaian Barang
              </h6>
            ...
            <form
                  ...
                  <div class="col-sm-6">
                    <div class="form-group">
                      <div class="row" style="heigth:30px">
                        <div class="col-sm-2 align-self-center">
                          <label for="material_requirement_id">No TTB</label>
                        </div>
                        <div class="col-sm-10 align-self-center">
                          <v-select
                          label="code"
                            ...
                            <div class="row">
                              ...
                              <div class="col-sm-10 align-self-center">
                                <input
                                  ...
                                  class="form-control form-control-sm"
                                  name="date"
                                  :value="form.pemakaian_barang_id ? form.pemakaian_barang_id.production_detail ? form.pemakaian_barang_id.production_detail.no_batch : '-' : '-'"
              ...
              <div class="table-striped">
                <table class="table table-sm mt-3">
                  ...
                  <tr
                    v-for="(item, index) in form.detail_retur_pemakaian_barang"
                    :key="index"
                  >
  <td class="table-transaction-row">
                    ...
                    <input
                      ...
                      <td class="table-transaction-row">
                        <money
                        v-model="item.quantity_pengajuan"
                        class="form-control"
                        @keydown.native="
                  ...
                  <tr v-if="!form.detail_retur_pemakaian_barang.length">
...
export default {
  ...
  async created() {
    try {
      if (this.isEditable) {
        let response = await this.$axios.get("inventory/retur_pemakaian_barang/" + this.id);

        Object.keys(this.form).forEach((item) => {
          if (item != "detail_retur_pemakaian_barang") {
            if(item === "pemakaian_barang_id"){
              this.form[item] = response.data.pemakaian_barang;
        ...
        this.form.detail_retur_pemakaian_barang = response.data.detail_retur_pemakaian_barang.map(
          (item) => {
            return {
              ...item,
      ...
      this.$router.push("/inventory/retur_pemakaian_barang");
    }
  },
  ...
  methods: {
    ...
    async onSearchProduction() {
      if (!this.isLoadingGetProduction) {
        ...
        await this.lookUp({
          url: "inventory/pemakaian_barang/get-pemakaian",
          lookup: "custom1",
          query:
            "?search=" + this.production_search +
            "&page=" + this.lookup_custom1.current_page +
            "&per_page=10&jenis="+ this.form.jenis,
        });
   ...
   onGetProces(search, isNext) {
    ...
    onSubmit(isInvalid) {
      ...
      if (!this.form.detail_retur_pemakaian_barang.length) {
      ...
      let url = "inventory/retur_pemakaian_barang";
      ...
      formData.detail_retur_pemakaian_barang = formData.detail_retur_pemakaian_barang.map((item) => {
        return {    
          quantity: item.quantity,
          quantity_pengajuan: item.quantity_pengajuan,
          product_id:
      ...
      this.$axios({
        ...
        .then((res) => {
          this.$toaster.success("Berhasil " + (this.isEditable ? "Update" : "Tambah") + " Return pemakaian barang");
    ...
    async setCode() {
      await this.getCode({
        url: "inventory/retur_pemakaian_barang",
        query: "",
      });
    ...
    addDetail() {      
      this.form.detail_retur_pemakaian_barang.push({
        product_id: "",
        quantity: 0.0,  
      });
    },
    ...
    deleteDetail(indexItem) {
      this.form.detail_retur_pemakaian_barang = this.form.detail_retur_pemakaian_barang.filter(
    ...
    onSetPemakaian(item) {
      if (item) {
        this.form.detail_retur_pemakaian_barang = item.detail_pemakaian_barangs.map(item => {
          return {
            grade_id: item.grade,
            product_id: item.product,
            description: "",
            quantity_pengajuan: item.quantity,
            quantity: 0.00
          }
        })
        console.log(item);
