...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "stock_out");
  ...
  methods : {
    ...
    onFormShow(){    
      this.$router.push("/inventory/stock_out/add");
    },

    onEdit(item){
      this.$router.push("/inventory/stock_out/"+item.id);
    },
    ...
    onPrintDetail(item){
      ...
      window.open(process.env.API_URL + "/inventory/stock_out/get-print-detail/" + item.id + "?token="+token, '_blank');    
    },   
     onRowSelected(index){
