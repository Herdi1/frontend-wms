                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <thead>
                    <tr>         
                      ...
                      <th>Dari Gudang</th>
                      <th>Ke Gudang</th>
                      <th @click="onSort('quantity',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer"
                        ...
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          ...
                          <div>
                            <i class="fas fa-caret-up"
                              :class="parameters.params.order == 'quantity' && parameters.params.sort == 'asc' ? '' : 'light-gray'"></i>
                            <i class="fas fa-caret-down"
                              :class="parameters.params.order == 'quantity' && parameters.params.sort == 'desc' ? '' : 'light-gray'"></i>
                          </div>
                        </div>
                      </th>      
                      <th>Keterangan</th>               
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>{{ item.warehouse_from ? item.warehouse_from.name : '-' }}</td>
                      <td>{{ item.warehouse_to ? item.warehouse_to.name : '-' }}</td>
                      <td>{{ item.quantity }}</td>
                      ...
                      <td class="text-center">
                        <div class="btn-group border border-success">                                                 
                          ...
                          <button class="btn btn-sm" 
                            @click="onEdit(item)"
                            v-if="!item.deleted_at && getRoles.update && item.is_last_mutation">
                            <i class="fas fa-pen text-warning"></i>
                          </button>
                          <button class="btn btn-sm" 
                            @click="onTrashed(item)" 
                            v-if="!item.deleted_at && getRoles.destroy && item.is_last_mutation">
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "stock_mutation");
  ...
  methods : {
    ...
    onFormShow(){    
      this.$router.push("/inventory/stock_mutation/add");
    },
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...
        warehouse_from_id : item.warehouse_from,
        warehouse_to_id : item.warehouse_to
      };
    ...
    onPrintDetail(item) {
      ...
      window.open(
        process.env.API_URL +
        "/inventory/stock_mutation/get-print-detail/" +
        item.id +
        "?token=" +
        token,
        "_blank"
