                    ...
                    <form @submit.prevent="() => console.log('Submit')"
                        ...
                        <div class="table-responsive table-striped">                        
                            <table class="table table-sm mt-3">
                                ...
                                <tr v-for="item,index in form.stock_broken_details"
                                    :key="index">
                                ...
                                <tr v-if="!form.stock_broken_details.length">
                                    <td colspan="7"
...
export default {
    ...
    async created(){
        try{        
            if(this.isEditable){
                let response = await this.$axios.get("inventory/stock_broken/"+this.id);

                Object.keys(this.form)
                    .forEach(item => {
                        if(item != "stock_broken_details"){
                ...
                this.form.stock_broken_details = response.data
                    .stock_broken_details
                    .map(item => {
                        return {
                            ...item,
            ...
            this.$router.push("/inventory/stock_broken");
        }
    },
    ...
    methods: {    
        ...
        async onSearchPurchaseingReturn(){
            if(!this.isLoadingGetPurchaseingReturn){
                ...
                await this.lookUp({    
                    url : "inventory/stock_broken/get-purchaseing-return",
        ...
        async onSearchWarehouse(){
            if(!this.isLoadingGetWarehouse){
                ...
                await this.lookUp({    
                    url : "inventory/stock_broken/get-warehouse",
                    lookup : "warehouses",
                    query : "?search="
        ...
        async onSearchProduct(){
            if(!this.isLoadingGetProduct){
                ...
                await this.lookUp({    
                    url : "inventory/stock_broken/get-product",
                    lookup : "products",
                    query : "?search="
                        + this.product_search + 
                        "&page=" 
        ...
        async onSearchChartOfAccount() {
            if (!this.isLoadingGetChartOfAccount) {
                ...
                await this.lookUp({
                url: "inventory/stock_broken/get-chart-of-account",
        ...
        onSubmit(isInvalid){
            ...
            if(!this.form.stock_broken_details.length){
                this.$toaster.error("Detail Stok Rusak Masih Kosong");
                return;
            }

            this.isLoadingForm = true;

            let url = "inventory/stock_broken";
            ...
            formData.stock_broken_details = formData
                .stock_broken_details
                .map(item => {
                    return {
                        ...item,
            ...
            this.$confirm({
                ...
                callback: async confirm => {         
                    if(confirm){
                        ...
                        window.open(
                            process.env.API_URL + "/inventory/stock_broken/get-print-detail/" + id + "?token=" + token
                            ,'_blank'
                        );    
                    } 
                }
        ...
        addDetail(){
            if(parseInt(this.limit_item_transaction) <= this.form.stock_broken_details.length){
                this.$toaster.error("Hanya " + this.limit_item_transaction + " batas item yang di perbolehkan");
                return;
            }

            this.form.stock_broken_details.push({
                product_id : '',
                quantity : 0.00,
                description : ''
            })
        ...
        deleteDetail(indexItem){
            this.form.stock_broken_details = this.form
                .stock_broken_details
        ...
        onSetProduct(index){            
            let details = [
                ...this.form.stock_broken_details
            ];

            let itemProduct = {
                ...this.form.stock_broken_details[index],            
            };
           
            if(!itemProduct.product_id) {
                this.form.stock_broken_details[index].product_id = '';
                return
            };
            ...
            if(products.length > 1){        
                ...
                this.form.stock_broken_details = details.filter((_,indexItem) => index != indexItem);            
            }
        },    
        ...
        onSetPurchaseingReturn(item){
            if(item){                         
                 this.form = {
                    ...this.form,                                                 
                    stock_broken_details : item.purchaseing_return_details 
                        .map(item => {
                            return {                            
                                product_id :  item.product,
