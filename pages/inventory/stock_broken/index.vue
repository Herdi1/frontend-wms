                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>{{ item.code }}</td>
                      <td>{{ item.date }}</td>   
                      <td>{{ item.purchaseing_return ? item.purchaseing_return.code : '-' }}</td>
                      ...
                      <td class="text-center">
                        <div class="btn-group border border-success">                     
                          ...
                          <button class="btn btn-sm" 
                            @click="onEdit(item)"
                            v-if="!item.deleted_at && getRoles.update && item.is_can_update_or_delete">
                            <i class="fas fa-pen text-warning"></i>
                          </button>
                          <button class="btn btn-sm" 
                            @click="onTrashed(item)" 
                            v-if="!item.deleted_at && getRoles.destroy && item.is_can_update_or_delete">
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "stock_broken");
  ...
  methods : {
    ...
    onFormShow(){    
      this.$router.push("/inventory/stock_broken/add");
    },

    onEdit(item){
      this.$router.push("/inventory/stock_broken/"+item.id);
    },
    ...
    onPrintDetail(item){
      ...
      window.open(process.env.API_URL + "/inventory/stock_broken/get-print-detail/" + item.id + "?token="+token, '_blank');    
    },   
     onRowSelected(index){
