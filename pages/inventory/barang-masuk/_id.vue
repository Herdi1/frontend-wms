          ...
          <div class="w-full grid grid-cols-2 grid-flow-row gap-2 mb-5">
            <div class="form-group" v-if="isEditable">
              <input-horizontal
                ...
                :required="true"
                v-model="parameters.form.kode_barang_masuk"
                :disabled="true"
                inputWidth="w-[60%]"
                labelWidth="w-[40%]"
              />
          ...
          <p class="text-xs font-medium">Tambah Kartu Stok</p>
                ...
                <div class="table-responsive overflow-y-hidden mb-7">
                  <table
                    ...
                    :class="
                      parameters.form.barang_masuk_details.length
                        ? 'mb-[300px]'
                        : ''
                    "
                  >
                      ...
                      >
                        ...
                        <td class="border border-gray-300 text-center">
                          <i
                            ...
                            @click="deleteDetailBarangMasuk(index)"
                          ></i>
                        </td>
                      </tr>
                      <tr
                        v-if="!parameters.form.barang_masuk_details.length > 0"
                      >
...
export default {
  ...
  methods: {
    ...
    async onSubmit(isInvalid) {
      ...
      this.isLoadingForm = true;
      let url = "inventory/barang-masuk";
      ...
      formData.barang_masuk_details =
        this.parameters.form.barang_masuk_details.map((item) => {
          return {
            ...item,
            barang_masuk_detail_id:
              typeof item.barang_masuk_detail_id === "object"
                ? item.barang_masuk_detail_id.barang_masuk_detail_id
                : "",
    ...
    onSelectValuation(item, index) {
      if (item) {
        this.parameters.form.barang_masuk_details[index].valuation_id = item;
    ...
    async onSelectZonaPlan(item, index) {
      if (item) {
        this.parameters.form.barang_masuk_details[index].zona_gudang_id = item;
        this.parameters.form.barang_masuk_details[
          index
        ].slot_penyimpanan_id_aisle = "";
        await this.onSearchSlotAisle(index);
      } else {
        this.parameters.form.barang_masuk_details[index].zona_gudang_id = "";
        this.parameters.form.barang_masuk_details[
          index
        ].slot_penyimpanan_id_aisle = "";
      }
      ...
      if (!this.isLoadingGetSlotRack) {
        ...
        await this.lookUp({
          ...
          query:
            ...
            this.parameters.form.barang_masuk_details[index]
    ...
    onSelectItemGudang(item, index) {
      if (item) {
        this.parameters.form.barang_masuk_details[index].item_gudang_id = item;
        this.parameters.form.barang_masuk_details[index].item_id = item.item_id;
      } else {
        this.parameters.form.barang_masuk_details[index].item_gudang_id = "";
      }
    },
    ...
    //add barang detail
    addBarangMasukDetail() {
      this.parameters.form.barang_masuk_details.push({
        item_id: "",
        item_gudang_id: "",
        item_pelanggan_id: "",
        shipment_id: "",
    ...
    deleteDetailBarangMasuk(index) {
      this.parameters.form.barang_masuk_details =
        this.parameters.form.barang_masuk_details.filter(
    ...
    addItem(item) {
      if (
        !this.parameters.form.barang_masuk_details.find(
        ...
        this.parameters.form.barang_masuk_details.push(detailItem);
      } else {
