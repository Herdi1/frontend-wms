                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <thead>
                    <tr>         
                      ...
                      <th>Nama</th> 
                      <th @click="onSort('system_stock',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer"
                        ...
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          ...
                          <div>
                            <i class="fas fa-caret-up"
                              :class="parameters.params.order == 'system_stock' && parameters.params.sort == 'asc' ? '' : 'light-gray'"></i>
                            <i class="fas fa-caret-down"
                              :class="parameters.params.order == 'system_stock' && parameters.params.sort == 'desc' ? '' : 'light-gray'"></i>
                          </div>
                        </div>
                      </th>      
                      <th @click="onSort('real_stock',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer"
                        style="min-width:100px">
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          <div>Stok Nyata</div>
                          <div>
                            <i class="fas fa-caret-up"
                              :class="parameters.params.order == 'real_stock' && parameters.params.sort == 'asc' ? '' : 'light-gray'"></i>
                            <i class="fas fa-caret-down"
                              :class="parameters.params.order == 'real_stock' && parameters.params.sort == 'desc' ? '' : 'light-gray'"></i>
                          </div>
                        </div>
                      </th>      
                       <th @click="onSort('difference_stock',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer"
                        style="min-width:100px">
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          <div>Selisih</div>
                          <div>
                            <i class="fas fa-caret-up"
                              :class="parameters.params.order == 'difference_stock' && parameters.params.sort == 'asc' ? '' : 'light-gray'"></i>
                            <i class="fas fa-caret-down"
                              :class="parameters.params.order == 'difference_stock' && parameters.params.sort == 'desc' ? '' : 'light-gray'"></i>
                          </div>
                        </div>
                      </th>      
                      <th>Keterangan</th>               
                      ...
                      <td class="text-center">
                        <div class="btn-group border border-success">                                                 
                          <button class="btn btn-sm" 
                            @click="onEdit(item)"
                            v-if="!item.deleted_at && getRoles.update && item.is_last_opname">
                            <i class="fas fa-pen text-warning"></i>
                          </button>
                          <button class="btn btn-sm" 
                            @click="onTrashed(item)" 
                            v-if="!item.deleted_at && getRoles.destroy && item.is_last_opname">
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "stock_opname");
  ...
  methods : {
    ...
    onFormShow(){    
      this.$router.push("/inventory/stock_opname/add");
    },
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...item,
        system_stock_now : 0.00,
        product_id : item.product,
        warehouse_id : item.warehouse
      };
      this.$refs["form-input"].onGetSystemStock();
