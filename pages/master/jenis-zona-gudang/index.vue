...
export default {
    ...
    getRoles() {
      if (this.user.is_superadmin == 1) {
        ...
        let main_role = this.user.role.menus.find(
          (item) => item.rute == "jenis-zona-gudang"
        );
  ...
  methods: {
    ...
    onFormShow() {
      this.$router.push("/master/jenis-zona-gudang/add");
    },

    onEdit(item) {
      this.$router.push(
        "/master/jenis-zona-gudang/" + item.jenis_zona_gudang_id
      );
    },
