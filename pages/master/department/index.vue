                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <thead>
                    <tr>         
                      ...
                      <th @click="onSort('name',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer" style="width:50%">
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          <div>Nama</div>
                          <div>
                            <i class="fas fa-caret-up"
                      ...
                      <th style="width:50%">Induk Proses</th>                                  
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>{{ item.code }}</td>
                      <td>{{ item.name }}</td>  
                      <td>{{item.proces_induk ? item.proces_induk.name : "-" }}</td>  
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "masterproses");
  ...
  methods : {
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...item,
        induk: item.data_induk,
        induk_2: item.data_induk_2,
        induk_3: item.data_induk_3,
        tipe_department_id: item.tipe_department,
    ...
    async onRestored(item){
      ...
      if(this.result == true){
        this.onLoad(this.parameters.params.page)
        this.$toaster.success("Data berhasil di restore")
      }else{
        this.$globalErrorToaster(this.$toaster,this.error);
      }
