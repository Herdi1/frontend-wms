                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>{{ item.date }}</td>
                      <td>                                            
                        Kode Po Supplier : {{ item.po_supplier ? item.po_supplier.code : '-' }}
                        <div v-if="item.po_supplier">
                          Supplier : {{ item.po_supplier.supplier ? item.po_supplier.supplier.name : "-" }}
                        </div>
                        ...
                        <div class="btn-group border border-success">
                          <button class="btn btn-sm"                        
                            ...
                            v-if="
                              ...
                              ( (item.po_supplier ? item.po_supplier.purchaseings_count : 0) == 0)
                            ">
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "purchaseing_dp");
  ...
  methods : {
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...item,
        po_supplier_id : item.po_supplier,     
