...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "po_supplier");
  ...
  methods : {
    ...
    onFormShow(){    
      this.$router.push("/purchaseing_module/po_supplier/add");
    },

    onEdit(id){
      this.$router.push("/purchaseing_module/po_supplier/"+id);
    },
    ...
    onPrintDetail(item){
      ...
      window.open(process.env.API_URL + "/purchaseing_module/po_supplier/get-print-detail/" + item.id + "?token="+token, '_blank');    
    },
