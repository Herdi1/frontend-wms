                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <thead>
                    <tr>         
                      ...
                      <th>Supplier</th>
                      <th @click="onSort('amount',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer">
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          <div>Jumlah</div>
                          <div>
                            <i class="fas fa-caret-up"
                              :class="parameters.params.order == 'amount' && parameters.params.sort == 'asc' ? '' : 'light-gray'"></i>
                            <i class="fas fa-caret-down"
                              :class="parameters.params.order == 'amount' && parameters.params.sort == 'desc' ? '' : 'light-gray'"></i>
                          </div>
                        </div>
                      </th>   
                      <th>Keterangan</th>                                                      
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>{{ item.date }}</td>
                      <td>                    
                        Kode Pembelian : {{ item.purchaseing ? item.purchaseing.code : '-'}}
                        <div v-if="item.purchaseing">
                          Supplier : {{ item.purchaseing.supplier ? item.purchaseing.supplier.name : '-'}}
                        </div>            
                        <div>
...
export default {
  ...
  data() {
    return {
      ...
      isPaginate          : true,
      parameters : {
        url : 'purchaseing_module/list_debt_payment',
        type : 'pdf',
        params :{
          soft_deleted : '',
          search      : '',
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "list_debt_payment");
  ...
  methods : {
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...item,
        purchaseing_id : item.purchaseing,    
      };
      window.$("#modal-form").modal("show");
      this.$refs["form-input"].$refs['form-validate'].reset();
    },
