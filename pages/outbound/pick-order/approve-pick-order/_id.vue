            ...
            <div class="grid grid-cols-1 md:grid-cols-2 w-full gap-2 mt-10">
              ...
              <div class="form-group flex items-start">
                ...
                <textarea
                  placeholder="Catatan"
                  class="w-1/2 pl-2 py-1 border rounded focus:outline-none"
                  v-model="parameters.form.catatan_approve"
                ></textarea>
              </div>
            </div>
          </div>
...
import ModalPickRequest from "../../../../components/transaksional/ModalPickRequest.vue";
import BiayaPickOrder from "../../../inventory/stok-opname/adjustment-stok-opname/BiayaPickOrder.vue";
export default {
  props: ["self"],
      ...
      handler(newVal) {
        newVal.forEach((item) => {
          if (item.dasar_perhitungan === "QTY") {
            ...
            item.total = item.jumlah * item.nominal_satuan * item.berat;
          } else if (item.dasar_perhitungan === "VOLUME") {
            item.total = item.jumlah * item.nominal_satuan * item.volume;
          } else {
            item.total = 0;
          }
        });
  ...
  async created() {
    ...
    try {
      ...
      if (this.isEditable) {
        ...
        if (!res.data.tagihan_pick_orders.length) {
          await Promise.all(
            this.parameters.form.pick_order_details.map((item) => {
              return this.$axios
          ...
          this.parameters.form.tagihan_pick_orders =
  ...
  methods: {
    ...
    async onSelectGudang(item) {
      if (item) {
        ...
        this.parameters.form.gudang_id = "";
        this.parameters.form.peralatan = "";
      }
    },
