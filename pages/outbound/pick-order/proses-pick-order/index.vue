...
export default {
  ...
  data() {
    return {
      title: "Proses Pick Order",
      isLoadingData: false,
      isPaginate: true,
      parameters: {
        url: "outbound/proses-pick-order",
        type: "pdf",
        params: {
          soft_deleted: "",
          search: "",
    ...
    getRoles() {
      if (this.user.is_superadmin == 1) {
        ...
        let main_role = this.user.role.menus.find(
          (item) => item.rute == "proses-pick-order"
        );
  ...
  methods: {
    ...
    onFormShow() {
      this.$router.push("/outbound/pick-order/proses-pick-order/add");
    },

    onEdit(item) {
      this.$router.push(
        "/outbound/pick-order/proses-pick-order/" + item.pick_order_id
      );
    },

    onDetail(item) {
      this.$router.push(
        "/outbound/pick-order/proses-pick-order/detail/" + item.pick_order_id
      );
    },
