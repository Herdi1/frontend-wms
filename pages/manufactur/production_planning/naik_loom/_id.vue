<template>
  ...
  <!-- {{form.detail_production_activities}} -->
        <div class="col-md-6">
          <div class="row">
            <div class="col-12">
              <div class="card">
...
export default {
  ...
  async created() {          
    ...
    if (this.isEditable) {    
      try {
        ...
        this.form.detail_production_activities = [];
        this.form.detail_production_activities.push({
                id: response.data.production_activity_loom.id,
                no_set: response.data.production_activity_loom.no_set,
                length: response.data.production_activity_loom.length,
                nett_weight: response.data.production_activity_loom.nett_weight,
                master_beam: response.data.production_activity_loom.master_beam,
                checked: true,
        }),

        // this.form.detail_production_activities = response.data.production_activity_loom.map(
        //   (item) => {
        //     return {
        //       ...item,
        //       id: item.id,
        ...
        // console.log(err);
        this.$router.push("/manufactur/production_planning/naik_loom");
      }
    }
  },
  ...
  methods: {
    ...
    async onChangeBatch() {
      let parent_id = 0;
      if (this.form.production_detail_id) {
        parent_id = this.form.production_detail_id.id;
      }
      await this.lookUp({
        url: "manufactur/production_activity/get-activity",
        lookup: "custom2",
        query: "?all=''&naik_loom=0&master_proces_id=33&order=no_set&sort=asc&production_detail_id=" + parent_id,
      });
      // try {
      ...
      if (!this.isLoadingGetMesin) {
        ...
        await this.lookUp({
          ...
          query:
            ...
            "&not_used=no&per_page=10&sort=asc&master_proces_id=16",
        });
    ...
    onSubmit(isInvalid) {
      ...
      // Hitung Wajib memilih 1 item untuk naik beam di mesin loom
      formData.detail_production_activities.filter((userliste) => {
        if (userliste.checked == true) {
          countData++
        }
      })
      if (countData == 0 || countData > 1) return this.$toaster.info("Wajib hanya boleh memilih 1 beam") ;
      // Hitung Wajib memilih 1 item untuk naik beam di mesin loom
