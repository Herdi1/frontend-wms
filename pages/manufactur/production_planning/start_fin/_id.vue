                ...
                <form
                  ...
                  <!-- <div class="form-group">
                    ...
                    <div class="row">
                      <div class="col-md-8">
                      <div class="form-group">
                      <label for="customer_id">No Pemakaian</label>
                        <v-select
                          label="code"
                          :loading="isLoadingGetPemakaian"
                          :options="lookup_custom3.data"
                          :filterable="false"
                          @search="onGetPemakaian"
                          @input="onChangePemakaian"
                          v-model="form.pemakaian_barang_id"
                        >
                          <template v-slot:option="option">
                          ...
                          <template #search="{ attributes, events }">
                            <input
                              class="vs__search"
                              :required="!form.pemakaian_barang_id"
                              v-bind="attributes"
                              v-on="events"
                            />
                          </template>
                          ...
                          >
                            <a
                              ...
                              @click="onGetPemakaian(search, false)"
                              class="flex-fill bg-primary text-white text-center"
                              href="#"
                              >Sebelumnya</a
                            >
                            <a
                              v-if="lookup_custom3.last_page >
                                ...
                                "
                              @click="onGetPemakaian(search, true)"
                              class="flex-fill bg-primary text-white text-center"
                              href="#"
                              >Selanjutnya</a
                            >
...
export default {
  ...
  data() {
    ...
    return {
      ...
      isStopSearchPemakaian: false,
      isLoadingGetPemakaian: false,
      pemakaian_search: "",
  ...
  methods: {
    ...
    onPrint(id) {
      ...
      window.open(
        process.env.API_URL +
        "/manufactur/production_activity/get-start-fin/" +
        id +
        "?token=" +
        token,
        "_blank"
    ...
    async onChangePemakaian() {
      //  let parent_id = 0;
      // if (this.form.pemakaian_barang_id) {
      //   parent_id = this.form.pemakaian_barang_id.id;
      // }
      // await this.lookUp({
      //   url: "manufactur/production_activity/get-activity",
      //   lookup: "custom2",
      //   query: "?master_proces_id=32&order=no_set&sort=asc&production_detail_id=" + parent_id,
      // });
      // try {
      // let response = await this.$axios.get("master/atribut_production_activity?order=id&sort=asc&master_proces_id=" + parent_id);
      this.form.detail_production_activities = this.form.pemakaian_barang_id.packing_list.detail_packing_list.map(
        (item) => {
          return {
            // ...item,
            // id: item.id,
            code: this.form.pemakaian_barang_id.packing_list.product.code,
            grade: this.form.pemakaian_barang_id.packing_list.grade.code,
            name: this.form.pemakaian_barang_id.packing_list.product.name,
            qty1: item.length,
            // unit: item.master_beam,
            qty2: item.weight,
            ref: item.production_activity.code,
    ...
    onGetPemakaian(search, isNext) {
      if (!search.length && typeof isNext === "function") return false;

      clearTimeout(this.isStopSearchPemakaian);

      this.isStopSearchPemakaian = setTimeout(() => {
        this.pemakaian_search= search;
        ...
        this.onSearchPemakaian();
      }, 600);
    },

    async onSearchPemakaian() {
      if (!this.isLoadingGetPemakaian) {
        this.isLoadingGetPemakaian = true;

        await this.lookUp({
          url: "inventory/pemakaian_barang/get-pemakaian-finish",
          lookup: "custom3",
          query:
            "?search=" + this.pemakaian_search +
            "&page=" + this.lookup_custom3.current_page +
            "&per_page=10&jenis=1&sort=asc",
        });

        this.isLoadingGetPemakaian = false;
      }
    },
      ...
      if (!this.isLoadingGetMesin) {
        ...
        await this.lookUp({
          ...
          query:
            ...
            "&per_page=10&sort=asc&master_proces_id="+this.form.proces_induk_id,
        });
      ...
      let formData = {
        ...
        production_detail_id:
          typeof this.form.production_detail_id == "object"
            ...
            : "",
        pemakaian_barang_id:
          typeof this.form.pemakaian_barang_id == "object"
            ? this.form.pemakaian_barang_id.id
            : "",
      ...
      this.$axios({
        ...
        .then(res => {
          ...
          if (this.isEditable) {
            ...
            let waktu = this.form.start_time;
