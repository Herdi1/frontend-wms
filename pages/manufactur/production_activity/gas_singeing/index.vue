...
export default {
  ...
  methods: {
    ...
    onFormShow() {
      this.$router.push("/manufactur/production_activity/gas_singeing/add");
    },

    onEdit(item) {
      this.$router.push("/manufactur/production_activity/gas_singeing/" + item.id);
    },
    ...
    onPrintDetail(item) {
      ...
      window.open(
        process.env.API_URL +
        "/manufactur/production_activity/get-print-summary-finish/" +
        item.id +
        "?token=" +
        token,
        "_blank"
