              ...
              <div class="row mt-2">                               
                <div class="col-md-12 float-right ">
                  <div class="receipt-right text-right">
                    ...
                    <p><b>Kode Produksi :</b> {{ parameters.form.production ? parameters.form.production.code : "-" }}</p>
                    <p><b>Gudang :</b> {{ parameters.form.warehouse ? parameters.form.warehouse.name : "-" }} </p>
                  </div>
                </div>
              </div>              
              <div>
                <table class="table table-striped table-sm vld-parent">
                  <thead>
                    <tr>
                      ...
                      <th style="width:5%">Qty Produksi</th>                    
                      <th style="width:10%">Qty Finish</th>  
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="item in parameters.form.sent_to_warehouse_details" :key="item.id">
                      <td>{{ item.product ? item.product.code : '-' }}</td>
                      <td>{{ item.product ? item.product.name : '-' }}</td>
                      <td>{{ item.quantity | formatPrice }}</td>
                      <td>{{ item.quantity_finish | formatPrice }}</td>
                    </tr>

                    <tr v-if="!parameters.form.sent_to_warehouse_details.length">
