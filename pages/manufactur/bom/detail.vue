                ...
                <table class="table table-striped table-sm vld-parent">
                  <thead>
                    <tr>
                      ...
                      <th style="width:5%">Standart Cost</th>
                      <th style="width:5%">Qty</th>
                      <th style="width:5%">Total Cost</th>                                  
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="item in parameters.form.bom_details" :key="item.id">
                      ...
                      <td>{{ (parseFloat(item.product ? item.product.cost_of_products_sold : 0) * item.quantity) | formatPrice }}</td>
                    </tr>

                    <tr v-if="!parameters.form.bom_details.length">
                    ...
                    <tr>
                      ...
                      <td>
                        <p>
                          <strong>{{ total_material | formatPrice }}</strong>
                        </p>
                      </td>
                    </tr>
...
export default {
  ...
  computed: {
    ...
    total_material(){
      let total_material = this.parameters.form.bom_details.reduce((prev,next) => {
        let total = parseFloat(next.product ? next.product.cost_of_products_sold : 0) * parseFloat(next.quantity);

        return parseFloat(prev) + parseFloat(total);
      },0);
