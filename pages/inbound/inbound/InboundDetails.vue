                    ...
                    >
                      <thead>
                        <tr class="text-sm uppercase text-nowrap">
                          ...
                          <th class="w-[200px] border border-gray-300" v-if="self.form.sumber_data === 'ASN'">
                            Dimensi
                          </th>
                          <th class="w-[200px] border border-gray-300" v-if="self.form.sumber_data === 'ASN'">
                            Lokasi Plan
                          ...
                          <th class="w-[300px] border border-gray-300" v-if="self.form.sumber_data === 'ASN'">
                            Alasan Beda Plan
                          </th>
                      ...
                      <tbody>
                        ...
                        >
                          ...
                          <td class="border border-gray-300" v-if="self.form.sumber_data === 'ASN'">
                            <div>
                              <p v-if="item.panjang">
  ...
  methods: {
    ...
    onDeleteDetails(index, item) {
      ...
      const notAllDeleted = this.self.form.inbound_details.some(
        (data) => data.item_gudang_id.item_gudang_id === item.item_gudang_id.item_gudang_id
      )
    ...
    async onSelectZona(item, index) {
      if (item) {
        this.self.form.inbound_details[
          index
        ].zona_gudang_id = item;
        this.self.form.inbound_details[
          index
      ...
      if (!this.isLoadingGetSlotAisle) {
        ...
        await this.lookUp({
          ...
          query:
            ...
            this.self.form.gudang_id.gudang_id +
            "&zona_gudang_id=" +
            this.self.form.inbound_details[index]
    ...
    sumQuantity(item){
      ...
      this.self.form.inbound_details.forEach((data) => {
        if(data.item_gudang_id.item_gudang_id == item.item_gudang_id.item_gudang_id){
          quantity = quantity + data.quantity
        }
      })
      const index = this.self.form.biaya_inbounds.findIndex(data => data.item_gudang_id == item.item_gudang_id.item_gudang_id)
