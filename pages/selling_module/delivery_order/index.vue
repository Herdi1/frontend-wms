                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <thead>
                    <tr>                                       
                      ...
                      <th style="width: 30%">Dikirim Ke</th>              
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" 
                      ...
                      <td>{{ item.date }}</td>
                      <td>
                        <div v-if="item.is_dropship">
                          Reseller : {{ item.reseller ? item.reseller.name : "-" }}
                          <br/>
                          <hr class="m-0 mt-1 p-0"/>
                          Pelanggan : {{ item.customer ? item.customer.name : "-" }}
                          <br/>
                          <hr class="m-0 mt-1 p-0"/>
                          Alamat : {{ item.customer ? item.customer.address : "-" }}
                        </div>
                        <div v-else>
                          Pelanggan : {{ item.customer ? item.customer.name : '-' }}
                          <br/>
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "delivery_order");
  ...
  methods : {
    ...
    onFormShow(){    
      this.$router.push("/selling_module/delivery_order/add");
    },

    onEdit(id){
      this.$router.push("/selling_module/delivery_order/"+id);
    },
    ...
    onPrintDetail(item){
      ...
      window.open(process.env.API_URL + "/selling_module/delivery_order/get-print-detail/" + item.id + "?token="+token, '_blank');    
    },
