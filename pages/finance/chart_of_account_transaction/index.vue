                  ...
                  <thead>
                    <tr>                                 
                      ...
                      <th @click="onSort('name',parameters.params.sort == 'asc' ? 'desc' : 'asc')"
                        class="cursor-pointer" style="width:40%">
                        <div class="d-flex flex-row justify-content-between align-items-baseline">
                          <div>Nama</div>
                          <div>
                            <i class="fas fa-caret-up"
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">                   
                      ...
                      <td>
                        {{ item.code }}
                        <br/>
                        Dibuat oleh : {{ item.user ? item.user.username : "-" }}
                      </td>
                      <td>{{ item.name }}</td>
                      <td>
                        <b>{{item.chart_of_account ? item.chart_of_account.name : "-" }}</b>
                      </td>
                      <td>                                                
                        <b>{{checkType(item.type)}}</b>
                      </td>                                      
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "chart_of_account_transaction");
  ...
  methods : {
    ...
    onFormShow(){
      ...
      this.$refs["form-input"].onSearchChartOfAccount();
      this.$refs["form-input"].rules = [];
    },
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...item,
        chart_of_account_id : item.chart_of_account ? item.chart_of_account : ""
      };
      window.$("#modal-form").modal("show");
      this.$refs["form-input"].$refs['form-validate'].reset();
      this.$refs["form-input"].onChangeType(false);
    },
    ...
    checkType(type){
      return this.types.find(item => item.value === type) ? this.types.find(item => item.value === type).title : '-';
    }
  }
}
</script>