                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>({{ item.master_proces ? item.master_proces.code : '-' }}) {{ item.master_proces ? item.master_proces.name : '-' }}</td>

                      <td>{{ item.cost | formatPrice}}</td>
                      <td>{{ item.periode || '-' }}</td>
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "cash_mutation");
