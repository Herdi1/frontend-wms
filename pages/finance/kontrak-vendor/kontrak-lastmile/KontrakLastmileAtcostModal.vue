      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header flex justify-between text-xl font-bold mb-3">
            <h5 class="modal-title">Tambah Kontrak Lastmile Atcost</h5>
          ...
          <div class="modal-body">
            ...
            >
              <form
                @submit.prevent="validate().then(() => onSubmitAtcost(invalid))"
              >
                <div class="grid grid-cols-2 w-[700px] gap-2 mt-10">
                  ...
                  <div class="form-grup flex items-start justify-between mb-5">
                    ...
                    <v-select
                      label="nama_lokasi"
                      :loading="self.isLoadingGetLokasi"
                      :options="self.lookup_mesin.data"
                      :filterable="false"
                      @search="self.onGetLokasi"
                    ...
                    >
                      ...
                      >
                        <span
                          ...
                          @click="self.onGetLokasi(search, false)"
                          class="flex-fill bg-primary text-white text-center cursor-pointer p-2 rounded"
                          >Sebelumnya</span
                        >
                        <span
                          ...
                          "
                          @click="self.onGetLokasi(search, true)"
                          class="flex-fill bg-primary text-white text-center cursor-pointer p-2 rounded"
                          >Selanjutnya</span
                        >
                      </li>
...
export default {
  ...
  methods: {
    async onSubmitAtcost(isInvalid) {
      if (isInvalid || this.isLoadingFormInsentifRitese) return;

      this.isLoadingFormInsentifRitese = true;
      let url;

      if (this.parameters.form.kontrak_lastmile_atcost_detail_id) {
        url = `finance/kontrak-lastmile/put-detail-kontrak-atcost/${this.parameters.form.kontrak_lastmile_atcost_detail_id}`;
      } else {
        url = `finance/kontrak-lastmile/post-detail-kontrak-atcost/${this.parameters.form.kontrak_lastmile_id}`;
      }

      let formData = {
        ...this.parameters.form,
        kontrak_lastmile_atcost_detail_id:
          typeof this.parameters.form.kontrak_lastmile_atcost_detail_id ===
          "object"
            ? this.parameters.form.kontrak_lastmile_atcost_detail_id
                .kontrak_lastmile_atcost_detail_id
            : this.parameters.form.kontrak_lastmile_atcost_detail_id,
        jenis_kontrak_id:
      ...
      this.$axios({
        method: this.parameters.form.kontrak_lastmile_atcost_detail_id
          ? "put"
          : "post",
        url: url,
        data: formData,
