      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header flex justify-between text-xl font-bold mb-3">
            <h5 class="modal-title">Tambah Kontrak Lastmile Insentif Jarak</h5>
          ...
          <div class="modal-body">
            <ValidationObserver
              ...
              ref="validateInsentifJarak"
            >
              <form
                @submit.prevent="
                  validate().then(() => onSubmitInsentifJarak(invalid))
                "
              >
                <div class="grid grid-cols-2 w-[700px] gap-2 mt-10">
                  ...
                  <div class="form-grup mb-5">
                    <input-koma
                      name="km_awal"
                      label="KM Awal"
                      :value="parameters.form.km_awal"
                      v-model="parameters.form.km_awal"
                  ...
                  <div class="form-grup mb-5">
                    <input-koma
                      name="km_akhir"
                      label="KM Akhir"
                      :value="parameters.form.km_akhir"
                      v-model="parameters.form.km_akhir"
                      isFlex="flex justify-between"
                    />
                  </div>
                  <div class="form-grup mb-5">
                ...
                <modal-footer-section
                  class="mt-5"
                  :isLoadingForm="isLoadingFormInsentifJarak"
                  @reset="formReset()"
                />
              </form>
            </ValidationObserver>
...
export default {
  ...
  data() {
    return {
      ...
      isLoadingFormInsentifJarak: false,
  ...
  methods: {
    async onSubmitInsentifJarak(isInvalid) {
      if (isInvalid || this.isLoadingFormInsentifJarak) return;

      this.isLoadingForm = true;
      let url;

      if (this.parameters.form.kontrak_lastmile_jarak_detail_id) {
        url = `finance/kontrak-lastmile/put-detail-kontrak-insentif-jarak/${this.parameters.form.kontrak_lastmile_jarak_detail_id}`;
      } else {
        url = `finance/kontrak-lastmile/post-detail-kontrak-insentif-jarak/${this.parameters.form.kontrak_lastmile_id}`;
      }

      let formData = {
        ...this.parameters.form,
        kontrak_lastmile_jarak_detail_id:
          typeof this.parameters.form.kontrak_lastmile_jarak_detail_id ===
          "object"
            ? this.parameters.form.kontrak_lastmile_jarak_detail_id
                .kontrak_lastmile_jarak_detail_id
            : this.parameters.form.kontrak_lastmile_jarak_detail_id,
        jenis_kontrak_id:
      ...
      this.$axios({
        method: this.parameters.form.kontrak_lastmile_jarak_detail_id
          ? "put"
          : "post",
        url: url,
        data: formData,
