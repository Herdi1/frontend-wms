<template>
  <div v-if="showModal">
    ...
    <div
      ...
      aria-hidden="true"
      id="modal-insentif-ritase"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header flex justify-between text-xl font-bold mb-3">
            <h5 class="modal-title">Tambah Kontrak Lastmile Insentif Ritase</h5>
          ...
          <div class="modal-body">
            <ValidationObserver
              ...
              ref="validateInsentifRitase"
            >
              <form
                @submit.prevent="
                  validate().then(() => onSubmitInsentifRitase(invalid))
                "
              >
                <div class="grid grid-cols-2 w-[700px] gap-2 mt-10">
                  ...
                  <div class="form-grup mb-5">
                    <input-koma
                      name="ritase_awal"
                      label="Ritase Awal"
                      :value="parameters.form.ritase_awal"
                      v-model="parameters.form.ritase_awal"
                  ...
                  <div class="form-grup mb-5">
                    <input-koma
                      name="ritase_akhir"
                      label="Ritase Akhir"
                      :value="parameters.form.ritase_akhir"
                      v-model="parameters.form.ritase_akhir"
                      isFlex="flex justify-between"
                    />
                  </div>
                  <div class="form-grup mb-5">
...
export default {
  ...
  methods: {
    async onSubmitInsentifRitase(isInvalid) {
      if (isInvalid || this.isLoadingFormInsentifRitese) return;

      this.isLoadingFormInsentifRitese = true;
      let url;

      if (this.parameters.form.kontrak_lastmile_ritase_detail_id) {
        url = `finance/kontrak-lastmile/put-detail-kontrak-insentif-ritase/${this.parameters.form.kontrak_lastmile_ritase_detail_id}`;
      } else {
        url = `finance/kontrak-lastmile/post-detail-kontrak-insentif-ritase/${this.parameters.form.kontrak_lastmile_id}`;
      }

      let formData = {
        ...this.parameters.form,
        kontrak_lastmile_ritase_detail_id:
          typeof this.parameters.form.kontrak_lastmile_ritase_detail_id ===
          "object"
            ? this.parameters.form.kontrak_lastmile_ritase_detail_id
                .kontrak_lastmile_ritase_detail_id
            : this.parameters.form.kontrak_lastmile_ritase_detail_id,
        jenis_kontrak_id:
      ...
      this.$axios({
        method: this.parameters.form.kontrak_lastmile_ritase_detail_id
          ? "put"
          : "post",
        url: url,
        data: formData,
