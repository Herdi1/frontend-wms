      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header flex justify-between text-xl font-bold mb-3">
            <h5 class="modal-title">Tambah Kontrak Lastmile Biaya BBM</h5>
          ...
          <div class="modal-body">
            <ValidationObserver
              ...
              ref="validateInsentifBerat"
            >
              <form
                @submit.prevent="
                  validate().then(() => onSubmitInsentifBerat(invalid))
                "
              >
                <div class="grid grid-cols-3 w-[700px] gap-2 mt-10">
                  ...
                  <div class="form-grup mb-5">
                    <input-koma
                      name="maksimal_volume"
                      label="Maksimal Volume"
                      :value="parameters.form.maksimal_volume"
                  ...
                  <div class="form-grup flex items-start justify-between mb-5">
                    ...
                    <v-select
                      disabled
                      label="nama_satuan"
                      :loading="self.isLoadingGetVolume"
                      :options="self.lookup_operator.data"
                      :filterable="false"
                      @search="self.onGetVolume"
                    ...
                    >
                      <li
                        ...
                        slot="list-footer"
                        class="p-1 border-t flex justify-between"
                        v-if="self.lookup_operator.data.length || search"
                      >
                        <span
                          v-if="self.lookup_operator.current_page > 1"
                          @click="self.onGetVolume(search, false)"
                          class="flex-fill bg-primary text-white text-center cursor-pointer p-2 rounded"
                          >Sebelumnya</span
                        >
                        <span
                          v-if="
                            self.lookup_operator.last_page >
                            self.lookup_operator.current_page
                          "
                          @click="self.onGetVolume(search, true)"
                          class="flex-fill bg-primary text-white text-center cursor-pointer p-2 rounded"
                          >Selanjutnya</span
                        >
                      </li>
...
export default {
  ...
  methods: {
    async onSubmitInsentifBerat(isInvalid) {
      if (isInvalid || this.isLoadingFormInsentifRitese) return;

      this.isLoadingFormInsentifRitese = true;
      let url;

      if (this.parameters.form.kontrak_lastmile_berat_detail_id) {
        url = `finance/kontrak-lastmile/put-detail-kontrak-insentif-berat/${this.parameters.form.kontrak_lastmile_berat_detail_id}`;
      } else {
        url = `finance/kontrak-lastmile/post-detail-kontrak-insentif-berat/${this.parameters.form.kontrak_lastmile_id}`;
      }

      let formData = {
        ...this.parameters.form,
        kontrak_lastmile_berat_detail_id:
          typeof this.parameters.form.kontrak_lastmile_berat_detail_id ===
          "object"
            ? this.parameters.form.kontrak_lastmile_berat_detail_id
                .kontrak_lastmile_berat_detail_id
            : this.parameters.form.kontrak_lastmile_berat_detail_id,
        kontrak_lastmile_berat_detail_id:
      ...
      this.$axios({
        method: this.parameters.form.kontrak_lastmile_berat_detail_id
          ? "put"
          : "post",
        url: url,
        data: formData,
