                    ...
                    <form @submit.prevent="() => console.log('Submit')"
                        ...
                        <div class="table-responsive table-striped">                        
                            <table class="table table-sm mt-3">
                                ...
                                <tr v-for="item,index in form.cash_in_details"
                                    :key="index">
                                ...
                                <tr v-if="!form.cash_in_details.length">
                                    <td colspan="5"
...
export default {
    ...
    async created(){
        try{        
            if(this.isEditable){
                let response = await this.$axios.get("finance/cash_in/"+this.id);

                Object.keys(this.form)
                    .forEach(item => {
                        if(item != "cash_in_details"){
                ...
                this.form.cash_in_details = response.data
                    .cash_in_details
                    .map(item => {
                        return {
                            ...item,
            ...
            this.$router.push("/finance/cash_in");
        }
    },
    ...
    methods: {    
        ...
        async onSearchChartOfAccount(){
            if(!this.isLoadingGetChartOfAccount){
                ...
                await this.lookUp({    
                    url : "finance/cash_in/get-chart-of-account",
        ...
        onSubmit(isInvalid){
            ...
            if(!this.form.cash_in_details.length){
                this.$toaster.error("Detail Penerminaan Kas Masih Kosong");
                return;
            }

            this.isLoadingForm = true;

            let url = "finance/cash_in";
            ...
            formData.cash_in_details = formData
                .cash_in_details
                .map(item => {
                    return {
        ...
        addDetail(){
            if(parseInt(this.limit_item_transaction) <= this.form.cash_in_details.length){
                this.$toaster.error("Hanya " + this.limit_item_transaction + " batas item yang di perbolehkan");
                return;
            }

            this.form.cash_in_details.push({
                chart_of_account_id : '',
                amount : 0.00,
                description : ''
            })
        ...
        deleteDetail(indexItem){
            this.form.cash_in_details = this.form
                .cash_in_details
        ...
        countTotal(){
            this.form.amount = this.form.cash_in_details.reduce((itemPrev,itemNext) => parseFloat(itemPrev) + parseFloat(itemNext.amount),0);
        },
        ...
        onSetChartOfAccountDetail(index){
            let details = [
                ...this.form.cash_in_details
            ];

            let item = {
                ...this.form.cash_in_details[index],            
            };
           
            if(!item.chart_of_account_id) {
                this.form.cash_in_details[index].chart_of_account_id = '';
                return
            };
            ...
            if(chartOfAccounts.length < 2){
                ...
                this.form.cash_in_details = details.filter((_,indexItem) => index != indexItem);            
            }
        }
    }
}
