                ...
                <table class="table table-striped table-sm vld-parent table-hover"
                  ...
                  <tbody>
                    <tr :class="{'table-active': ActiveRow == i }" v-for="(item,i) in data" :key="i"  @click="onRowSelected(i)">
                      ...
                      <td>{{ item.date }}</td>
                      <td>                                            
                        Kode Po Pelanggan : {{ item.po_customer ? item.po_customer.code : '-' }}
                        <div v-if="item.po_customer">
                          Pelanggan : {{ item.po_customer.customer ? item.po_customer.customer.name : "-" }}
                        </div>
                        ...
                        <div class="btn-group border border-success">
                          <button class="btn btn-sm"                        
                            ...
                            v-if="
                              ...
                              ( (item.po_customer ? item.po_customer.sellings_count : 0) == 0)
                            ">
...
export default {
  ...
  computed : {
    ...
    getRoles(){
      if(!this.user.parent_id){
        ...
        let main_role = this.user.group_role.roles.find(item => item.name == "selling_dp");
  ...
  methods : {
    ...
    onEdit(item){
      ...
      this.$refs["form-input"].parameters.form = {
        ...item,
        po_customer_id : item.po_customer,    
